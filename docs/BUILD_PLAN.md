...
6. The Foundry: a submission pipeline (GitHub-based) for Behaviors, rule-based or exported policies. CI runs, in order: interface and schema validation, a determinism and purity check (the Behavior imports nothing forbidden and is side-effect free), and a seed-based ladder gauntlet where the submission plays a fixed set of matches against a baseline roster on the deterministic core. Only submissions that validate and clear a baseline bar are eligible to merge. Results are reproducible from seeds. (DoD note: implemented as `packages/foundry` -- stage 1 manifest/schema validation and entry load, stage 2 a static purity scan plus a same-run idempotence check, stage 3 a 25-fixed-seed gauntlet against a shape-specific baseline roster via the core's `runMatchWithBehaviors` seam, with a win-rate bar calibrated from measured data. `pnpm foundry:validate <submission-dir>` runs all three stages and exits 0/nonzero; CI's `foundry:validate` steps gate `submissions/sample-aggro` and `submissions/sample-policy` on every push, and the foundry vitest suite -- run by `pnpm -r test` -- proves rejection of a deliberately invalid submission at each stage, plus reproducibility of the gauntlet itself.)
