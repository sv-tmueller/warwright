...
| `QUEUE_MAX_FAILURES` | no | `3` | Bounded-eviction: a queue entry is evicted (not restored) once a failed-pairing restore would bring its failure count to this value; must be at least 1. See Matchmaking queue below. |
| `QUEUE_MAX_AGE_MS` | no | `60000` | Bounded-eviction: a queue entry is evicted once its total time in the pool, at a failed-pairing restore, would reach this many ms; must be a positive integer. See Matchmaking queue below. |
...
Configurable via env vars (see Environment variables above): `QUEUE_WINDOW_MS` (default `5000`; the client polls `GET /queue` at 2s, so perceived wait is roughly `window + 2s`) and `QUEUE_MAX_POOL` (default `8`, which also bounds a pass's synchronous CPU burst — `resolveMatch` runs a full headless sim per pairing, so K/2 back-to-back resolves at most). A lone waiter gets `202 { status: 'waiting' }` immediately (that *is* "told none is available yet"), can `DELETE /queue`, and is guaranteed to pair within `QUEUE_WINDOW_MS` of a second player joining (or instantly once the pool reaches K). There is no forced idle-time queue timeout for a healthy waiter in this slice — the bounded-eviction policy below is scoped to entries that have actually failed a pairing, not to how long a never-yet-paired waiter has been in the pool.
...
**Bounded eviction for repeatedly-failing pairings (#144).** Under the symmetric `failPairing` restore above, an entry whose pairing keeps failing (e.g. its user row was deleted mid-flight, or `resolveMatch`/`applyMatchRatings` hits a persistent error) would otherwise be restored to `waiting` and retried every window forever — a ghost entry. Instead, each restore increments the entry's failure count, and the entry is evicted (dropped from the pool instead of restored) once either bound is reached: its failure count hits `QUEUE_MAX_FAILURES` (default `3`), or its total time in the pool since its original enqueue reaches `QUEUE_MAX_AGE_MS` (default `60000`, a generous backstop for an entry that keeps failing without quickly reaching the failure-count cap in a busy pool). Both checks run only when a pairing has actually failed — a healthy waiter who simply hasn't found a partner yet is never evicted by either bound, no matter how long they've waited (failure-count/failed-attempt-age scoped, not idle-time scoped). An eviction logs a warning (`app.log.warn`, via the queue's `onEviction` hook) and needs no change to `GET /queue`: an evicted entry is simply no longer present in the queue's internal state, so the existing status read reports `idle` for it on the next poll, and a subsequent `DELETE /queue` 404s. Because `failPairing` restores/evicts both sides of a failed pairing symmetrically, an innocent opponent repeatedly unlucky enough to pair with the same ghost entry can also eventually be evicted — an accepted consequence of the both-restore design, bounded by the same caps.
